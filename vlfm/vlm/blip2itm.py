...
class BLIP2ITM:
    ...
    def cosine(self, image: np.ndarray, txt: str) -> float: # 暂时没用
        """
...
class BLIP2ITMClient:
    # def __init__(self, port: int = 12082):
    def __init__(self, port: int = main_args.blip_port):
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--port", type=int, default=12082)
    parser.add_argument("--port", type=int, default=main_args.blip_port)
    ...
    class BLIP2ITMServer(ServerMixin, BLIP2ITM):
        def process_payload(self, payload: dict) -> dict:
            ...
            return {"response": self.cosine(image, payload["txt"])} # 需要调用
