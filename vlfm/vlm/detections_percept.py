...
class ObjectDetections:
    ...
    def __add__(self, other: "ObjectDetections") -> "ObjectDetections":
        """Add two ObjectDetections instances by concatenating their attributes."""
        # 检查类型
        if not isinstance(other, ObjectDetections):
            raise TypeError(f"Cannot add ObjectDetections with {type(other)}")

        # 合并 boxes 和 logits（沿第0维拼接）
        combined_boxes = torch.cat([self.boxes, other.boxes], dim=0)
        combined_logits = torch.cat([self.logits, other.logits], dim=0)

        # 合并 phrases
        combined_phrases = self.phrases + other.phrases

        # 返回新实例（image_source设为None，_annotated_frame重置）
        return ObjectDetections(
            boxes=combined_boxes,
            logits=combined_logits,
            phrases=combined_phrases,
            image_source=self.image_source,  
            fmt="xyxy",  # 因为初始化时已经转换过
        )
