...
class VLFMTrainer(PPOTrainer):
    ...
    # load训练好的RL模型
    policy_model = HistoryAwareUNet(in_channels=6).to("cuda")
    policy_model.load_state_dict(torch.load("Models_train_PPO/policy/multi_process_sac/1143_actor"))
    policy_model.eval()
    ...
    def _eval_checkpoint(
        ...
        logger_file_name = "./log_files/log_for_sac_hm3d_v1_IL_all_vision_RL_model_1143_"+date_time+f"_init_{main_args.init_episode}"
        ...
        VLFMTrainer.rgb_ls, VLFMTrainer.depth_ls = get_rgb_image_ls(habitat_env)
