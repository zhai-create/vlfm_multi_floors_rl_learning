...
from vlfm.sac_floors.SACD import SACD_agent
...
class VLFMTrainer(PPOTrainer):
    ...
    now_model_id = 25 # critic_sample暂时不要
    ...
    def _eval_checkpoint(
        ...
        selected_episodes = selected_episodes[main_args.process_id::20]
        # selected_episodes = selected_episodes[5:] # 0-->5 # 中断修改 # 5, 1
        ...
        while len(stats_episodes) < (number_of_eval_episodes * evals_per_ep):
                ...
                if(action_data is not None): # 说明还在一些coner case阶段，初始化将继续
                    ...
                    if not not_done_masks[0].item(): # 如果当前回合已经结束
                        ...
                        if math.isfinite(episode_stats["distance_to_goal"]) and math.isfinite(VLFMTrainer.episode_reward):
                            ...
                            real_index = len(all_spl_ls)+247 # 中断修改
                            ...
                            writer.add_scalar("Result/success_num", num_successes+194, real_index) # 中断修改
                            ...
                            writer.add_scalar("Result/spl_mean", (np.sum(all_spl_ls)+0.5014*247)/real_index, real_index) # 中断修改
                            # writer.add_scalar("Result/spl_mean", np.mean(all_spl_ls), real_index)
                            writer.add_scalar("Result/ne_mean", (np.sum(all_ne_ls)+2.831*247)/real_index, real_index) # 中断修改
            ...
            if (max_id>VLFMTrainer.now_model_id):
                ...
                while True:
                    try:
                        VLFMTrainer.floor_policy.actor.load_state_dict(checkpoint)
                        break
                    except:
                        time.sleep(0.1)
                        continue
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item(): # 如果环境 i 中的一个回合已结束
                    ...
                    if ((VLFMTrainer.floor_select_steps%50)==0) and (action_data is None) and (VLFMTrainer.current_action is not None):
                        now_geodesic_dis = episode_stats["distance_to_goal"]
                ...
                if(VLFMTrainer.current_action is not None) and (action_data is None) and ((VLFMTrainer.floor_select_steps%50)==0 or (not not_done_masks[i].item())) and (math.isfinite(rl_step_reward)): # action_space为空，不加入buffer中，不记录rl_reward，记录episode_reward
