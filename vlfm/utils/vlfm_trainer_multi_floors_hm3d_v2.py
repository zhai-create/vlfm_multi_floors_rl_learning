...
def get_stairs_id(habitat_env):
    current_scene_dir = "data/scene_datasets/hm3d_v0.2/val/"+habitat_env.current_episode.scene_id.split('/')[-2]+"/"
...
class VLFMTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        # logger_file_name = "./log_files/log_ascent_actor_multi_stairs_RL_model_1002_multi_revise_hm3d_v2_"+date_time+f"_init_{main_args.init_episode}"
        logger_file_name = "./log_files/multi_floors_cnt_hm3d_v2_"+date_time+f"_init_{main_args.init_episode}"
        ...
        # 2. 判断object_goal是否与机器人在同一层
        while True:
            is_feasible = 0
            for temp_goal in habitat_env.current_episode.goals: # 如果没有object_goal的高度位置不在机器人初始化的楼层中，则结束当前episode
                height = temp_goal.position[1]
                is_feasible += is_on_same_floor(
                    height=height, episode=habitat_env.current_episode
                ) 
                if(is_feasible>0):
                    break
            if(is_feasible == 0):
                writer.add_scalar("Result/multi_floor_tag", 1, index_in_episodes+1)
            else:
                writer.add_scalar("Result/multi_floor_tag", 0, index_in_episodes+1)      

            index_in_episodes += 1
            
            
            if(index_in_episodes>=len(selected_episodes)):
                break
