...
import numpy as np
import copy
from vlfm.policy.unet_model_multi_floors import MultiFloorNavigator, MultiFloor_Double_Q_Net
import torch.nn.functional as F
from torch.distributions.categorical import Categorical
from vlfm.sac_floors.arguments import args as q_args
...
class SACD_agent(object):
    def __init__(self, args):
        ...
        self.actor = MultiFloorNavigator(pretrained_path="Models_train_PPO_one_floor/policy/multi_process_sac/1002_actor").to(args.device)
        self.actor_il = MultiFloorNavigator(pretrained_path="Models_train_PPO_one_floor/policy/multi_process_sac/1002_actor").to(args.device)
        ...
        # checkpoint = torch.load("Models_train_PPO/policy/multi_process_sac/1261_actor")
        # self.actor.load_state_dict(checkpoint)
        ...
        # self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.actor_lr)
        # checkpoint = torch.load("Models_train_PPO/policy/multi_process_sac/1261_actor_optimizer")
        # self.actor_optimizer.load_state_dict(checkpoint)
        # ===================

        self.init_T = 10
        self.q_critic = MultiFloor_Double_Q_Net(pretrained_path="Models_train_PPO_one_floor/policy/multi_process_sac/1002_actor", device=args.device).to(args.device)
        ...
        # load训练好的模型(rl)
        self.q_critic_optimizer = torch.optim.Adam(self.q_critic.parameters(), lr=self.lr)
        checkpoint = torch.load("Models_train_PPO/policy/multi_process_sac/25_critic_optimizer")
        self.q_critic_optimizer.load_state_dict(checkpoint)
        
        checkpoint = torch.load("Models_train_PPO/policy/multi_process_sac/25_critic")
        self.q_critic.load_state_dict(checkpoint)

        checkpoint = torch.load("Models_train_PPO/policy/multi_process_sac/25_critic_target")
    ...
    def choose_action(self, state):
        with torch.no_grad():
            ...
            final_floor_mask = final_floor_mask.unsqueeze(0).to(self.device)
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            preds = self.actor(state_tensor)
            ...
            # 应用softmax得到概率分布
            softmax_probs = F.softmax(torch.from_numpy(masked_pred), dim=0)
            sampled_index = Categorical(softmax_probs).sample()
            return np.array([sampled_index.item()]) # 待检查--> RL中, action直接按照index来算
    ...
    def learn(self, batch):
        ...
        for i in range(num_mini_batches):
            ...
            for i in range(logits.size(0)):
                logits_map = logits[i] # (a_dim, )

                # floor_mask数据
                final_floor_mask = [1, 1, 1]
                if (np.sum(mini_batch['state'][i, :6, :, :].cpu().numpy()[5])==0) or (np.sum(mini_batch['state'][i, :6, :, :].cpu().numpy()[5])!=0 and np.sum(mini_batch['state'][i, :6, :, :].cpu().numpy()[4])!=0 and np.sum(mini_batch['state'][i, :6, :, :].cpu().numpy()[3])==0):
                    final_floor_mask[0] = 0
                if (np.sum(mini_batch['state'][i, 6:12, :, :].cpu().numpy()[4])!=0 and np.sum(mini_batch['state'][i, 6:12, :, :].cpu().numpy()[3])==0):
                    final_floor_mask[1] = 0
                if (np.sum(mini_batch['state'][i, 12:18, :, :].cpu().numpy()[5])==0) or (np.sum(mini_batch['state'][i, 12:18, :, :].cpu().numpy()[5])!=0 and np.sum(mini_batch['state'][i, 12:18, :, :].cpu().numpy()[4])!=0 and np.sum(mini_batch['state'][i, 12:18, :, :].cpu().numpy()[3])==0):
                    final_floor_mask[2] = 0

                curr_mask_map = np.array(final_floor_mask)
                curr_mask_map = torch.tensor(curr_mask_map, device=self.device)
            ...
            for i in range(logits_il.size(0)):
                logits_map_il = logits_il[i]  # 不再使用detach()
