...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def get_best_matched_cluster(self, cluster_ls, target_class):
        max_score = 0
        max_cluster = None

        for temp_cluster in cluster_ls:
            temp_score = temp_cluster.object_dict_per_cluster[target_class].pc_num*temp_cluster.object_dict_per_cluster[target_class].confidence_score
            ...
            if ((len(self._observations_cache["frontier_sensor"])+np.sum(self._observations_cache["semantic_map"]))>0):
                ...
                if(self.last_value_map is None):
                    if(len(self.reliable_cluster_ls)>0):
                        best_matched_cluster = self.get_best_matched_cluster(self.reliable_cluster_ls, self._target_object)
                ...
                if(now_semantic_map[pred_row, pred_col]>0) and (len(self.reliable_cluster_ls)>0):
                    mode = "navigate"
    ...
    def _update_object_map(
        ...
        self.reliable_cluster_ls = []
        suspected_cluster_ls = [] # 只要该cluster中存在object_goal就add
        for temp_cluster in self._object_map._all_object_cluster_ls:
            ...
            if(best_matched_class==self._target_object):
                ...
                suspected_cluster_ls.append(temp_cluster)
            elif (self._target_object in temp_cluster.object_dict_per_cluster):
                suspected_cluster_ls.append(temp_cluster)

        self._object_map.get_semantic_map(self.reliable_cluster_ls, self._target_object)

        if ((len(self._observations_cache["frontier_sensor"])+np.sum(self._object_map.semantic_map))==0):
            self.reliable_cluster_ls = suspected_cluster_ls
            self._object_map.get_semantic_map(self.reliable_cluster_ls, self._target_object)
