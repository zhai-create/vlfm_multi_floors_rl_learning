...
from vlfm.mapping.object_point_cloud_map import ObjectPCDetect
...
def get_all_clusters_pc(cluster_ls, target_class):
    res_pc = []
    for temp_cluster in cluster_ls:
        temp_pc = temp_cluster.object_dict_per_cluster[target_class].pc.copy()
        if(len(temp_pc)==0):
            continue
        else:
            if(len(res_pc)==0):
                ...
                res_pc = np.vstack((res_pc, temp_pc))
    return res_pc
...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def act(
        ...
        if (self.is_reverse == True):
        ...
        elif not self._done_initializing:  # Initialize
            mode = "initialize"
            ...
            if (self.is_need_closer==True):
            ...
            elif ((len(self._observations_cache["frontier_sensor"])+np.sum(self._observations_cache["semantic_map"]))>0):
                # =====> 准备数据 <=====
                # frontier_map
                ...
                semantic_frontier_map[now_frontier_map == 1] = -1
                semantic_frontier_map[now_semantic_map != 0] = now_semantic_map[now_semantic_map != 0] # 1
                ...
                if(self.last_value_map is None):
                    ...
                    if(goal is None):
                        ...
                        if (now_semantic_map[pixel_points[0, 1], pixel_points[0, 0]] != 0):
                            target_map[pixel_points[:, 1], pixel_points[:, 0]] = 1 # 计算语义地图（1为相应类别）
                        else:
                            now_semantic_map_one_position = np.argwhere(now_semantic_map != 0)
                ...
                '''
                # resize_gai
                if(resize_semantic_frontier_map[pred_row, pred_col]>0):
                    mode = "navigate"    

                    # # filter_state  
                    # navigate_target_cloud = get_all_clusters_pc(self.reliable_cluster_ls+self.suspected_cluster_ls, self._target_object)
                    
                    navigate_target_cloud_cluster_ls = []
                    for temp_cluster in self._object_map._all_object_cluster_ls:
                        if(temp_cluster.has_in_semantic_map==True):
                            navigate_target_cloud_cluster_ls.append(temp_cluster)
                    navigate_target_cloud = get_all_clusters_pc(navigate_target_cloud_cluster_ls, self._target_object)
    ...
    def _pre_step(self, observations: "TensorDict", masks: Tensor) -> None: # 根据条件执行reset, 获得当前帧观测信息(存在_observations_cache中)，self._policy_info为空
        ...
        try:
            ...
            for temp_i in range(len(self._observations_cache["frontier_sensor"])):
                should_add = True
            ...
            self._observations_cache["frontier_sensor"] = np.array(now_right_frontier_ls) # 当前帧最终真正需要的frontier
    ...
    def _get_other_object_detections(self, img):
        ...
        # # object_no_action_revise（不用）
        # if(self._target_object in self._same_meaning_list):
        #     dino_target_classes.append(self._target_object)
    ...
    def _update_object_map(
        ...
        # # filter_state
        # self._object_map.get_semantic_map(self.reliable_cluster_ls+self.suspected_cluster_ls, self._target_object)
