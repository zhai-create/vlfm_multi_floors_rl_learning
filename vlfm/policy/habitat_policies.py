...
class HabitatMixin:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._cx, self._cy = image_width // 2, (config.habitat.simulator.agents.main_agent.sim_sensors.rgb_sensor.height // 2)
    ...
    def act(
        ...
        try:
            action, rnn_hidden_states = parent_cls.act(obs_dict, rnn_hidden_states, prev_actions, masks, deterministic) # 执行BaseObjectNavPolicy中的act
    ...
    def select_action(self):
        parent_cls: BaseObjectNavPolicy = super()  # type: ignore
        action = parent_cls.select_action()
        return PolicyActionData(
                actions=action,
    ...
    #     # 根据当前观察，得到rgb，depth，frontiers，xy，yaw等信息
    ...
    #         self._obstacle_map.update_agent_traj(robot_xy, camera_yaw) # 将robot_xy添加至_camera_positions， camera_yaw赋值给_last_camera_yaw
    #     else:
    ...
    #         "nav_depth": observations["depth"],  # for pointnav (原始depth)
    #         "robot_xy": robot_xy,
    ...
    #     # 与操作
    #     observations["explored_area"] = self._obstacle_map.explored_area
    #     observations["navigable_map"] = self._obstacle_map._navigable_map
    #     observations["obstacle_map"] = self._obstacle_map._map
    ...
    def _cache_observations(self: Union["HabitatMixin", BaseObjectNavPolicy], observations: TensorDict) -> None:
        ...
        for temp_index in range(4):
            if(temp_index==0):
                ...
                x, y = observations["gps"][0].cpu().numpy()
                current_camera_position = observations["compass"][0].cpu().item()

                if(temp_index==1): # 2
                    other_angle_camera_yaw = new_normalize_angle(current_camera_position+0.5*np.pi)
                elif(temp_index==2): # 3
                    other_angle_camera_yaw = new_normalize_angle(current_camera_position+np.pi)
                elif(temp_index==3): # 4
                    other_angle_camera_yaw = new_normalize_angle(current_camera_position-0.5*np.pi)
                ...
                camera_position = np.array([x, -y, self._camera_height])
                robot_xy = camera_position[:2]
                other_angle_tf_camera_to_episodic = xyz_yaw_to_tf_matrix(camera_position, other_angle_camera_yaw)
                self._obstacle_map.update_map(
                    other_angle_depth,
