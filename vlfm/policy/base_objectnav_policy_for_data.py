...
def compare_dicts(dict1, dict2):
    ...
    if(np.array_equal(dict1["robot_xy"], dict2["robot_xy"])) and (np.array_equal(dict1["pixel_points"], dict2["pixel_points"])) and (dict1["robot_yaw"]==dict2["robot_yaw"]):
        return True
    else:
        return False
...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.last_frontier_map = None
        self.last_semantic_map = None
        self.last_value_map = None
        self.last_xy_map = None
        self.last_sin_map = None
        self.last_cos_map = None

        self.all_save_dict_ls = []
        self.all_temp_dict_ls = []
    ...
    def act(
        ...
        if ((len(self._observations_cache["frontier_sensor"])+np.sum(self._observations_cache["semantic_map"]))>0) and (mode!="initialize"):
            # frontier_map
            ...
            # value_map
            now_value_map = observations['value_map']
            ...
            if(self.last_target_pos is None):
                self.last_target_pos = copy.deepcopy(pixel_points)
                self.last_target_map = copy.deepcopy(target_map)
                
                self.last_frontier_map = copy.deepcopy(now_frontier_map)
                self.last_semantic_map = copy.deepcopy(now_semantic_map)
                self.last_value_map = copy.deepcopy(now_value_map)
                self.last_xy_map = copy.deepcopy(now_xy_map)
                self.last_sin_map = copy.deepcopy(now_sin_map)
                self.last_cos_map = copy.deepcopy(now_cos_map)

                self.last_robot_xy = copy.deepcopy(robot_xy)
                self.last_robot_yaw = copy.deepcopy(robot_yaw)


            save_dict = {
                "robot_xy": robot_xy, "robot_yaw": robot_yaw,
                "last_robot_xy": self.last_robot_xy, "last_robot_yaw": self.last_robot_yaw,
                "now_target_pos":pixel_points, "now_target_map":target_map, "now_frontier_map": now_frontier_map, "now_semantic_map": now_semantic_map, "now_value_map":now_value_map, "now_xy_map":now_xy_map, "now_sin_map":now_sin_map, "now_cos_map":now_cos_map,
                "last_target_pos":self.last_target_pos, "last_target_map":self.last_target_map, "last_frontier_map": self.last_frontier_map, "last_semantic_map": self.last_semantic_map, "last_value_map":self.last_value_map, "last_xy_map":self.last_xy_map, "last_sin_map":self.last_sin_map, "last_cos_map":self.last_cos_map
            }
            # 判断当前字典是否存储
            temp_save_dict = {"robot_xy": robot_xy, "robot_yaw": robot_yaw, "pixel_points": pixel_points}
            if(len(self.all_temp_dict_ls)==0):
                # np.save('{}/{}.npy'.format(save_root, len(os.listdir(save_root))+1+36877), save_dict)
                self.all_save_dict_ls.append(save_dict)
                self.all_temp_dict_ls.append(temp_save_dict)
            else:
                is_repeated = False
                for temp_dict in self.all_temp_dict_ls:
                    if compare_dicts(temp_dict, temp_save_dict)==True:
                        is_repeated = True
                        break
            ...
            print("robot_xy_save:", robot_xy, robot_yaw, pixel_points)
            self.last_target_pos = copy.deepcopy(pixel_points)
            self.last_target_map = copy.deepcopy(target_map)
        ...
        return pointnav_action, rnn_hidden_states, self.all_save_dict_ls
