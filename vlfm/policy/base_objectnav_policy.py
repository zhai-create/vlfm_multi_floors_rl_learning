...
from prior_stair import up_floor_dict, down_floor_dict
...
from vlfm.utils.geometry_utils import (
    ...
    transform_points,
    within_fov_cone,
    get_current_world_pos,
    get_absolute_pos_world,
    get_relative_pos_world
...
def compare_dicts(dict1, dict2):
    ...
    if(np.array_equal(dict1["robot_xy"], dict2["robot_xy"])) and (dict1["robot_yaw"]==dict2["robot_yaw"]) and (dict1["robot_height"]==dict2["robot_height"]):
        return True
    else:
        return False

def get_goal_floor(goal_height, robot_floor_height, ceiling_height=2.0):
    """
    ...
    """
    if (goal_height >= robot_floor_height) and (goal_height < robot_floor_height + ceiling_height):
        return 0 # 在本层
    elif(goal_height < robot_floor_height):
        return -1 # 在下面一层
    else:
        return 1 # 在上面一层
...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.predict_floor_action = 0
        self.is_in_current_floor_flag = False

        self.flash_stair_steps = 0 # newest_debug: 瞬移跨楼层
        self.has_zero_pitch = 0
        self.end_climb_rot_world = None
        self.end_climb_point_world = None

        self.curr_floor_semantic_frontier_map = None
        self.curr_floor_now_value_map = None
    ...
    def _process_stair_climb_state(self, robot_xy: np.ndarray, robot_px: np.ndarray, stair_map: np.ndarray, climb_direction: int):
        ...
        if not self._reach_stair:
            ...
            elif (not self.is_robot_in_stair_map_fast(robot_px, stair_map)[0]) or (self.has_zero_pitch==1): # newest_debug: 瞬移跨楼层
                self._reset_stair_climb_state()
                self._climb_stair_over = True
    ...
    def _update_obstacle_map(self, observations_cache, current_step_stairs_res, pitch_angle) -> None:
        ...
        if (0+1) in self.current_step_stairs_res:
            stair_mask = self.current_step_stairs_res[1]
        else:
            stair_mask = []
    ...
    def get_model_input_data_for_multi_floor(self, floor_index, robot_xy, robot_yaw, observations):
        # =====> 准备数据 <=====
        # frontier_map
        ...
        if(self._value_map_list[floor_index].last_value_map is None):
            if (len(self._all_floor_frontier_list[floor_index])+np.sum(now_semantic_map))>0: # 如果是第一帧决策，且action_space不为空
                # =====> 第一帧启发式target_map <=====
                ...
                # =====> 第一帧启发式target_map <=====
                self._value_map_list[floor_index].last_semantic_frontier_map = copy.deepcopy(semantic_frontier_map)
                self._value_map_list[floor_index].last_value_map = copy.deepcopy(now_value_map)
                self._value_map_list[floor_index].last_target_map = copy.deepcopy(target_map)
            else:
                self._value_map_list[floor_index].last_semantic_frontier_map = copy.deepcopy(semantic_frontier_map)
                self._value_map_list[floor_index].last_value_map = copy.deepcopy(now_value_map)
                self._value_map_list[floor_index].last_target_map = np.zeros((1000, 1000), dtype=np.float32)

        # resize_gai


        resize_last_semantic_frontier_map = compress_map_ultrafast_not_zero(self._value_map_list[floor_index].last_semantic_frontier_map)
        resize_last_value_map = compress_map_ultrafast_not_zero(self._value_map_list[floor_index].last_value_map)
        resize_last_target_map = compress_map_ultrafast_not_zero(self._value_map_list[floor_index].last_target_map)
    ...
    def get_model_input_data_for_one_floor(self, floor_index, robot_xy, robot_yaw, observations):
        # =====> 准备数据 <=====
        # frontier_map
        if(len(self._all_floor_frontier_list[floor_index])>0):
            now_frontier_map = np.zeros((1000, 1000))
        ...
        if(VLFMTrainer.current_floor_steps==0):
            # =====> 第一帧启发式target_map <=====
            target_map = np.zeros((1000, 1000), dtype=np.float32)
            goal = self._get_target_object_location(self._object_map_list[floor_index], robot_xy) # 如果看到了goal，则返回其最佳位置；否则返回None
            ...
            last_semantic_frontier_map = self._value_map_list[floor_index].last_semantic_frontier_map
            last_value_map = self._value_map_list[floor_index].last_value_map
            last_target_map =self._value_map_list[floor_index].last_target_map
    ...
    def get_model_input_data_for_one_floor_only_semantic_value(self, floor_index, robot_xy, robot_yaw):
        # =====> 准备数据 <=====
        # frontier_map
        if(len(self._all_floor_frontier_list[floor_index])>0):
            now_frontier_map = np.zeros((1000, 1000))
        ...
        # value_map
        now_value_map = self._value_map_list[floor_index]._value_map_for_vis[:, :, 0]
        return semantic_frontier_map, now_value_map
    ...
    def choose_multi_floor_action(self, state):
        VLFMTrainer.floor_policy.eval()
        with torch.no_grad():
            down_floor_input_data = state[:6]
            current_floor_input_data = state[6:12]
            up_floor_input_data = state[12:18]

            # floor_mask数据
            final_floor_mask = [1, 1, 1]
            if (np.sum(down_floor_input_data[5])==0) or (np.sum(down_floor_input_data[5])!=0 and np.sum(down_floor_input_data[4])!=0 and np.sum(down_floor_input_data[3])==0):
                final_floor_mask[0] = 0
            if (np.sum(current_floor_input_data[4])!=0 and np.sum(current_floor_input_data[3])==0):
                final_floor_mask[1] = 0
            if (np.sum(up_floor_input_data[5])==0) or (np.sum(up_floor_input_data[5])!=0 and np.sum(up_floor_input_data[4])!=0 and np.sum(up_floor_input_data[3])==0):
                final_floor_mask[2] = 0

            final_floor_mask = torch.FloatTensor(np.array(final_floor_mask))
            final_floor_mask = final_floor_mask.unsqueeze(0).to("cuda")
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to("cuda")

            preds = VLFMTrainer.floor_policy(state_tensor)

            pred_map = preds[0].detach().cpu().numpy()
            mask_map = final_floor_mask[0].cpu().numpy()
            ...
            # 应用softmax得到概率分布
            softmax_probs = F.softmax(torch.from_numpy(masked_pred), dim=0).numpy()
            softmax_pred = softmax_probs.reshape(masked_pred.shape)
    ...
    def select_action(self):
        if(VLFMTrainer.current_floor_steps%5)==0:
            VLFMTrainer.current_action, pred_row, pred_col = VLFMTrainer.rl_policy.choose_action(VLFMTrainer.current_state)
        else:
            # resize_gai
            resize_last_target_map = compress_map_ultrafast_not_zero(self._value_map_list[self._cur_floor_index].last_target_map)
            rows, cols = np.where(resize_last_target_map == 1)
            pred_row, pred_col = rows[0], cols[0]
            VLFMTrainer.current_action = np.array([pred_row, pred_col])

        self.is_in_current_floor_flag = True
        # resize_gai
        ...
        # print("pred_row, pred_col:", pred_row, pred_col)
        # print("sub_goal_array:", sub_goal_array)


        '''
        if(self.now_semantic_map[pred_row, pred_col]>0):
        '''
        # resize_gai
        resize_semantic_frontier_map = VLFMTrainer.current_state[3]
        if(resize_semantic_frontier_map[pred_row, pred_col]==1):
            ...
            if(len(navigate_target_cloud)>0):    
                real_sub_goal_array, _ = find_closest_vector(sub_goal_array, navigate_target_cloud[:, :2])
            ...
            if(len(explore_target_cloud)>0):  
                mode = "explore"  
                real_sub_goal_array, _ = find_closest_vector(sub_goal_array, explore_target_cloud[:, :2])
        ...
        # 只需要更新当前楼层的即可
        self._value_map_list[self._cur_floor_index].last_semantic_frontier_map = copy.deepcopy(self.curr_floor_semantic_frontier_map)
        self._value_map_list[self._cur_floor_index].last_value_map = copy.deepcopy(self.curr_floor_now_value_map)
        self._value_map_list[self._cur_floor_index].last_target_map = copy.deepcopy(last_target_map)



        # 最后的最后
        VLFMTrainer.current_floor_steps += 1
        ...
        self._policy_info.update(self._get_policy_info([])) # 每act一次，有一个新的_policy_info
        self._num_steps += 1
    ...
    def act(
        ...
        if(self.flash_stair_steps>0) and (self.flash_stair_steps<20): # newest_debug: 瞬移跨楼层(跨完楼层后消耗20步)
            detections = [[]]
            if(self.has_zero_pitch==1):
                # 感知stair
                self.current_step_stairs_res = {}
                ...
                self.has_zero_pitch += 1

            elif(self.has_zero_pitch>1):
                if (self._pitch_angle>0):
                    pointnav_action = TorchActionIDs.LOOK_DOWN
                    self._pitch_angle -= self._pitch_angle_offset
                ...
                if (self._pitch_angle>0):
                    ...
                    # 旋转180度
                    rot_old = R.from_quat(self.end_climb_rot_world)
                    rot_increment = R.from_euler('y', 180, degrees=True)
                    rot_new = rot_increment * rot_old
                    q_new_coeffs = rot_new.as_quat()
                    end_climb_quat_world = quat_from_coeffs(q_new_coeffs[::-1]) # 旋转完毕
                    VLFMTrainer.habitat_env._sim.set_agent_state(np.array(self.end_climb_point_world), end_climb_quat_world) # 完成瞬移

                    pointnav_action = TorchActionIDs.TURN_LEFT


            self.flash_stair_steps += 1
            mode = "flash_climbing"
        ...
        current_scene_id_number = VLFMTrainer.habitat_env.current_episode.scene_id.split('/')[-2].split('-')[0]
        if(current_scene_id_number not in up_floor_dict):
            self.current_step_stairs_res = {}
        ...
        # newest_debug: 检查当前到底是否具备上下楼功能
        current_scene_id_number = VLFMTrainer.habitat_env.current_episode.scene_id.split('/')[-2].split('-')[0]
        if(current_scene_id_number in up_floor_dict):
            up_dict_per_scene = up_floor_dict[current_scene_id_number]
            down_dict_per_scene = down_floor_dict[current_scene_id_number]
            world_cx, world_cy, world_cz, world_turn = get_current_world_pos(VLFMTrainer.habitat_env)
            if(len(up_dict_per_scene["position"])==1):
                with_up_floor_dis = abs(down_dict_per_scene["position"][0][1]-world_cz)
                with_down_floor_dis = abs(up_dict_per_scene["position"][0][1]-world_cz)
                if(with_up_floor_dis<with_down_floor_dis): # 说明机器人目前距离上面那层更近，隶属于上面那层，只能下楼
                    self._obstacle_map._up_stair_frontiers = np.array([])
                else:
                    self._obstacle_map._down_stair_frontiers = np.array([])
            elif(len(up_dict_per_scene["position"])==2):
                low_floor_height = up_dict_per_scene["position"][0][1]
                mid_floor_height = up_dict_per_scene["position"][1][1]
                high_floor_height = down_dict_per_scene["position"][1][1]
                diff_height_dis_ls = [abs(world_cz-low_floor_height), abs(world_cz-mid_floor_height), abs(world_cz-high_floor_height)]
                min_diff_height_dis = min(diff_height_dis_ls)
                min_diff_height_index = diff_height_dis_ls.index(min_diff_height_dis)
                if(min_diff_height_index==0): # 说明与最下层相距近，隶属于最下层，只能上楼
                    self._obstacle_map._down_stair_frontiers = np.array([])
                elif(min_diff_height_index==2):  # 说明与最上层相距近，隶属于最上层，只能下楼
                    self._obstacle_map._up_stair_frontiers = np.array([])
        ...
        start_climb_point_world = None
        end_climb_point_world = None
        end_climb_rot_world = None # 只存储原始信息，如果要用需要转180度
        
        # newest_debug: 瞬移跨楼层(提前判断是否到达stair口)
        candidate_mode_ls = ["get_close_to_stair", "NO ACTION, GO UP FLOOR", "NO ACTION, GO DOWN FLOOR", "PRIOR_UP_OR_DOWN", "GO UP FLOOR", "GO DOWN FLOOR", "look_down", "look_down_twice", "look_up"]
        if(self.last_mode in candidate_mode_ls) and (self._climb_stair_flag in [1, 2]):
            current_scene_id_number = VLFMTrainer.habitat_env.current_episode.scene_id.split('/')[-2].split('-')[0]
            up_dict_per_scene = up_floor_dict[current_scene_id_number]
            down_dict_per_scene = down_floor_dict[current_scene_id_number]
            world_cx, world_cy, world_cz, world_turn = get_current_world_pos(VLFMTrainer.habitat_env)
            ...
            if(self._climb_stair_flag==1):
                min_height_dis = 10000000000
                current_floor_index = None
                for temp_index in range(len(up_dict_per_scene["position"])):
                    temp_height_dis = abs(up_dict_per_scene["position"][temp_index][1]-world_cz)
                    if(temp_height_dis<min_height_dis):
                        min_height_dis = temp_height_dis
                        current_floor_index = temp_index
                start_climb_point_world = up_dict_per_scene["position"][current_floor_index]
                
                if ((start_climb_point_world[2]-world_cx)**2+(start_climb_point_world[0]-world_cy)**2)**0.5<1.0:
                    self._climb_stair_over = False
                    self._reach_stair = True
                    self._climb_stair_paused_step = 0 # 确保一定能跨楼层
                    end_climb_point_world = down_dict_per_scene["position"][current_floor_index]
                    end_climb_rot_world = down_dict_per_scene["rotation"][current_floor_index]
                ...
                min_height_dis = 10000000000
                current_floor_index = None
                for temp_index in range(len(down_dict_per_scene["position"])):
                    temp_height_dis = abs(down_dict_per_scene["position"][temp_index][1]-world_cz)
                    if(temp_height_dis<min_height_dis):
                        min_height_dis = temp_height_dis
                        current_floor_index = temp_index
                start_climb_point_world = down_dict_per_scene["position"][current_floor_index]
                
                if ((start_climb_point_world[2]-world_cx)**2+(start_climb_point_world[0]-world_cy)**2)**0.5<1.0:
                    self._climb_stair_over = False
                    self._reach_stair = True
                    self._climb_stair_paused_step = 0 # 确保一定能跨楼层
                    end_climb_point_world = up_dict_per_scene["position"][current_floor_index]
                    end_climb_rot_world = up_dict_per_scene["rotation"][current_floor_index]
            ...
            if self._reach_stair: # 到达了楼梯
                if self._pitch_angle == 0 and self._climb_stair_flag == 2:
                    ...
                    if self._obstacle_map._climb_stair_paused_step < 30:
                        ...
                        # newest_debug: 瞬移跨楼层
                        if(end_climb_point_world is None): # 寻找目前在哪个楼梯口
                            current_scene_id_number = VLFMTrainer.habitat_env.current_episode.scene_id.split('/')[-2].split('-')[0]
                            up_dict_per_scene = up_floor_dict[current_scene_id_number]
                            down_dict_per_scene = down_floor_dict[current_scene_id_number]
                            world_cx, world_cy, world_cz, world_turn = get_current_world_pos(VLFMTrainer.habitat_env)
                            ...
                            if(self._climb_stair_flag==1):
                                ...
                                current_floor_index = None
                                for temp_index in range(len(up_dict_per_scene["position"])):
                                    temp_dis = ((up_dict_per_scene["position"][temp_index][1]-world_cz)**2+(up_dict_per_scene["position"][temp_index][2]-world_cx)**2+(up_dict_per_scene["position"][temp_index][0]-world_cy)**2)**0.5
                                    if(temp_dis<min_dis):
                                        min_dis = temp_dis
                                        current_floor_index = temp_index
                                end_climb_point_world = down_dict_per_scene["position"][current_floor_index]
                                ...
                                current_floor_index = None
                                for temp_index in range(len(down_dict_per_scene["position"])):
                                    temp_dis = ((down_dict_per_scene["position"][temp_index][1]-world_cz)**2+(down_dict_per_scene["position"][temp_index][2]-world_cx)**2+(down_dict_per_scene["position"][temp_index][0]-world_cy)**2)**0.5
                                    if(temp_dis<min_dis):
                                        min_dis = temp_dis
                                        current_floor_index = temp_index
                                end_climb_point_world = up_dict_per_scene["position"][current_floor_index]
                                end_climb_rot_world = up_dict_per_scene["rotation"][current_floor_index]
                                self._climb_stair_paused_step = 0
                        self.end_climb_rot_world = end_climb_rot_world
                        self.end_climb_point_world = end_climb_point_world


                        # 直接调整到新的map
                        self._reach_stair_centroid = True

                        if (self._pitch_angle>1e-5):
                            self._pitch_angle -= self._pitch_angle_offset
                            pointnav_action = TorchActionIDs.LOOK_DOWN
                        elif(self._pitch_angle<-(1e-5)):
                            self._pitch_angle += self._pitch_angle_offset
                            pointnav_action = TorchActionIDs.LOOK_UP
                        else:
                            self.has_zero_pitch += 1
        ...
        current_floor_input_data = None
        up_floor_input_data = None
        down_floor_input_data = None
        ...
        if(pointnav_action is None) or (pointnav_action==TorchActionIDs.OTHER):
            ...
            # 不随便低头
                ...
                if (num_frontiers+num_semantic)>0:  
                    ...
                    # 当前层存储数据
                    current_floor_input_data, _, _, _ = self.get_model_input_data_for_multi_floor(self._cur_floor_index, self._observations_cache["robot_xy"], self._observations_cache["robot_heading"], observations)
                    ...
                    if(self._obstacle_map._explored_up_stair) and (self._obstacle_map._has_up_stair):
                        up_floor_input_data, _, _, _ = self.get_model_input_data_for_multi_floor(self._cur_floor_index+1, self._obstacle_map._up_stair_end, np.pi, observations)
                    elif(len(real_up_stair_frontiers)>0) and (self._obstacle_map._has_up_stair):
                        up_floor_input_data, _, _, _ = self.get_model_input_data_for_multi_floor(self._cur_floor_index+1, real_up_stair_frontiers[0], np.pi, observations)
                    else:
                        up_floor_resize_last_semantic_frontier_map = np.zeros((500, 500), dtype=np.float32)
                        up_floor_resize_last_value_map = np.zeros((500, 500), dtype=np.float32)
                        up_floor_resize_last_target_map = np.zeros((500, 500), dtype=np.float32)
                        up_floor_resize_semantic_frontier_map = np.zeros((500, 500), dtype=np.float32)
                        up_floor_resize_now_value_map = np.zeros((500, 500), dtype=np.float32)
                        up_floor_resize_robot_state_map = np.zeros((500, 500), dtype=np.float32)

                        up_floor_input_data = np.stack([
                            up_floor_resize_last_semantic_frontier_map, up_floor_resize_last_value_map, up_floor_resize_last_target_map,
                            up_floor_resize_semantic_frontier_map, up_floor_resize_now_value_map, up_floor_resize_robot_state_map
                        ], axis=0)
                        up_floor_input_data = torch.FloatTensor(up_floor_input_data)
                    ...
                    if(self._obstacle_map._explored_down_stair) and (self._obstacle_map._has_down_stair):
                        down_floor_input_data, _, _, _ = self.get_model_input_data_for_multi_floor(self._cur_floor_index-1, self._obstacle_map._down_stair_end, np.pi, observations)
                    elif(len(real_down_stair_frontiers)>0) and (self._obstacle_map._has_down_stair):
                        down_floor_input_data, _, _, _ = self.get_model_input_data_for_multi_floor(self._cur_floor_index-1, real_down_stair_frontiers[0], np.pi, observations)
                    else:
                        down_floor_resize_last_semantic_frontier_map = np.zeros((500, 500), dtype=np.float32)
                        down_floor_resize_last_value_map = np.zeros((500, 500), dtype=np.float32)
                        down_floor_resize_last_target_map = np.zeros((500, 500), dtype=np.float32)
                        down_floor_resize_semantic_frontier_map = np.zeros((500, 500), dtype=np.float32)
                        down_floor_resize_now_value_map = np.zeros((500, 500), dtype=np.float32)
                        down_floor_resize_robot_state_map = np.zeros((500, 500), dtype=np.float32)

                        down_floor_input_data = np.stack([
                            down_floor_resize_last_semantic_frontier_map, down_floor_resize_last_value_map, down_floor_resize_last_target_map,
                            down_floor_resize_semantic_frontier_map, down_floor_resize_now_value_map, down_floor_resize_robot_state_map
                        ], axis=0)

                        down_floor_input_data = torch.FloatTensor(down_floor_input_data)
                

                    # VLFMTrainer.next_state = np.concatenate((
                    #     down_floor_input_data.cpu().numpy(), current_floor_input_data.cpu().numpy(), up_floor_input_data.cpu().numpy()
                    # ), axis=0)

                    multi_floor_state = np.concatenate((
                        down_floor_input_data.cpu().numpy(), current_floor_input_data.cpu().numpy(), up_floor_input_data.cpu().numpy()
                    ), axis=0)

                    if(VLFMTrainer.floor_select_steps%50)==0:
                        self.predict_floor_action = self.choose_multi_floor_action(multi_floor_state)

                    VLFMTrainer.floor_select_steps += 1
                    ...
                    # 如果本层完蛋怎么办
                    if (self.predict_floor_action==0) and ((len(self._all_floor_frontier_list[self._cur_floor_index])+np.sum(self._object_map.semantic_map))==0): # 如果当前楼层action_space为空，则尝试先上后下
                        if (((self._obstacle_map._explored_up_stair) and (self._obstacle_map._has_up_stair)) or ((len(real_up_stair_frontiers)>0) and (self._obstacle_map._has_up_stair))):
                            self.predict_floor_action = 1
                        elif (((self._obstacle_map._explored_down_stair) and (self._obstacle_map._has_down_stair)) or ((len(real_down_stair_frontiers)>0) and (self._obstacle_map._has_down_stair))):
                            self.predict_floor_action = -1
                        else:
                            self.predict_floor_action = 100
                        
                    # 如果上层完蛋怎么办
                    elif (self.predict_floor_action==1) and (not (((self._obstacle_map._explored_up_stair) and (self._obstacle_map._has_up_stair)) or ((len(real_up_stair_frontiers)>0) and (self._obstacle_map._has_up_stair)))): 
                        # 优先本层
                    ...
                    # 如果下层完蛋怎么办
                    elif (self.predict_floor_action==-1) and (not (((self._obstacle_map._explored_down_stair) and (self._obstacle_map._has_down_stair)) or ((len(real_down_stair_frontiers)>0) and (self._obstacle_map._has_down_stair)))):
                        # 优先本层
                        if ((len(self._all_floor_frontier_list[self._cur_floor_index])+np.sum(self._object_map.semantic_map))>0):
                            self.predict_floor_action = 0
                        elif (((self._obstacle_map._explored_up_stair) and (self._obstacle_map._has_up_stair)) or ((len(real_up_stair_frontiers)>0) and (self._obstacle_map._has_up_stair))):
                            self.predict_floor_action = 1
                        else:
                            self.predict_floor_action = 100

                    if(self.predict_floor_action==1) and (((self._obstacle_map._explored_up_stair) and (self._obstacle_map._has_up_stair)) or ((len(real_up_stair_frontiers)>0) and (self._obstacle_map._has_up_stair))): # 上楼
                        mode = "GO UP FLOOR"
                        curr_floor_semantic_frontier_map, curr_floor_now_value_map = self.get_model_input_data_for_one_floor_only_semantic_value(self._cur_floor_index, self._observations_cache["robot_xy"], self._observations_cache["robot_heading"])
                        if(len(real_up_stair_frontiers)>0):
                            up_floor_start_pos = real_up_stair_frontiers
                        else:
                            up_floor_start_pos = np.array([np.array([f[0], f[1]+0.05]) for f in self._obstacle_map._up_stair_frontiers])
                        # pointnav_action = self._navigate_stair_if_unexplored_floor(direction="up")
                        pointnav_action = self._direct_navigate_stair(direction="up", real_stair_frontiers=up_floor_start_pos)
                        ...
                        # 只需要更新当前楼层的即可
                        self._value_map_list[self._cur_floor_index].last_semantic_frontier_map = copy.deepcopy(curr_floor_semantic_frontier_map)
                        self._value_map_list[self._cur_floor_index].last_value_map = copy.deepcopy(curr_floor_now_value_map)

                        return pointnav_action, rnn_hidden_states

                    elif(self.predict_floor_action==-1) and (((self._obstacle_map._explored_down_stair) and (self._obstacle_map._has_down_stair)) or ((len(real_down_stair_frontiers)>0) and (self._obstacle_map._has_down_stair))): # 下楼
                        mode = "GO DOWN FLOOR"
                        curr_floor_semantic_frontier_map, curr_floor_now_value_map = self.get_model_input_data_for_one_floor_only_semantic_value(self._cur_floor_index, self._observations_cache["robot_xy"], self._observations_cache["robot_heading"])
                        if(len(real_down_stair_frontiers)>0):
                            down_floor_start_pos = real_down_stair_frontiers
                        else:
                            down_floor_start_pos = np.array([np.array([f[0], f[1]+0.05]) for f in self._obstacle_map._down_stair_frontiers])
                        # pointnav_action = self._navigate_stair_if_unexplored_floor(direction="down")
                        pointnav_action = self._direct_navigate_stair(direction="down", real_stair_frontiers=down_floor_start_pos)
                    ...
                    elif(self.predict_floor_action==0) and ((len(self._all_floor_frontier_list[self._cur_floor_index])+np.sum(self._object_map.semantic_map))>0): # 留在本层
                        self._climb_stair_over = True
                        input_data, curr_mask, curr_floor_semantic_frontier_map, curr_floor_now_value_map = self.get_model_input_data_for_one_floor(self._cur_floor_index, self._observations_cache["robot_xy"], self._observations_cache["robot_heading"], self._observations_cache)
                        
                        self.curr_floor_semantic_frontier_map = curr_floor_semantic_frontier_map
                        self.curr_floor_now_value_map = curr_floor_now_value_map

                        VLFMTrainer.next_state = input_data
                        
                        return None, rnn_hidden_states
                    ...
                    if pointnav_action is not None:
                        ...
                        self.last_mode = mode
                        return pointnav_action, rnn_hidden_states
        else: # (pointnav_action is not None) and (pointnav_action!=TorchActionIDs.OTHER)
            if(mode == "look_for_downstair") or (mode == "get_close_to_stair"):
                num_frontiers = np.sum([len(temp_floor_frontiers) for temp_floor_frontiers in self._all_floor_frontier_list])
                num_semantic = np.sum([np.sum(temp_semantic_map.semantic_map) for temp_semantic_map in self._object_map_list])
        ...
        if(self.is_in_current_floor_flag==False):
            VLFMTrainer.current_floor_steps = 0
