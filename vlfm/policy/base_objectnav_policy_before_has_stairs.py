    ...
    def act(
        ...
        for temp_index in range(4):
            if(temp_index==0):
                ...
                other_angle_object_map_rgbd = [(VLFMTrainer.rgb_ls[temp_index-1].cpu().numpy(), other_angle_depth, other_angle_tf_camera_to_episodic, self._observations_cache["object_map_rgbd"][0][3], self._observations_cache["object_map_rgbd"][0][4], self._observations_cache["object_map_rgbd"][0][5], self._observations_cache["object_map_rgbd"][0][6])]

                other_angle_detections = [
                    self._update_other_angle_object_map(rgb, depth, tf, min_depth, max_depth, fx, fy) # 完成当前rgb检测，累计记录当前帧rgb上检测得到的所有mask结果，将当前rgb观测+当前detect_mask对应的object_name对应的点云累计到self.clouds中+过滤掉clouds中“在当前fov及dis范围下， 不正常（随机数不为1）的点云”
                    for (rgb, depth, tf, min_depth, max_depth, fx, fy) in other_angle_object_map_rgbd
                ] # get::当前帧的semantic_map # get::当前帧的机器人位置
