...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.now_semantic_map = None
        self.detections = None
        self.semantic_frontier_map = None
        self.now_value_map = None
    ...
    def select_action(self):
        VLFMTrainer.current_action, pred_row, pred_col = VLFMTrainer.rl_policy.choose_action(VLFMTrainer.current_state, VLFMTrainer.epsilon)
        ...
        self.last_semantic_frontier_map = copy.deepcopy(self.semantic_frontier_map)
        self.last_value_map = copy.deepcopy(self.now_value_map)
        ...
        self._policy_info.update(self._get_policy_info(self.detections[0])) # 每act一次，有一个新的_policy_info
        self._num_steps += 1
    ...
    def act(
        ...
        if not self._done_initializing:  # Initialize
            ...
            print("==========> <==========")
            return pointnav_action, rnn_hidden_states, self._done_initializing
            # ============> <============
        else:
            if ((len(self._observations_cache["frontier_sensor"])+np.sum(self._observations_cache["semantic_map"]))>0):
                ...
                self.now_semantic_map = now_semantic_map
                ...
                self.semantic_frontier_map = semantic_frontier_map

                # value_map
                now_value_map = observations['value_map'][:, :, 0]
                self.now_value_map = now_value_map
                ...
                return None, rnn_hidden_states, self._done_initializing
