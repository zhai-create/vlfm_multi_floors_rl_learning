...
class SumTree(object):
    ...
    def update(self, data_index, priority): # 更新树的过程
    ...
    def get_index(self, v): # 根据随机数，得到buffer中样本的过程
    ...
    def get_priority_min_index(self):
        copy_tree = copy.deepcopy(self.tree)
        min_val = copy_tree[self.buffer_capacity - 1:].min()
        min_index = np.where(copy_tree[self.buffer_capacity - 1:] == min_val)[0][0]
        return min_index