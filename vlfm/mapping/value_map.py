...
class ValueMap(BaseMap):
    ...
    def __init__(
        ...
        self._value_map_for_vis = np.zeros((size, size, value_channels), np.float32)
    ...
    def reset(self) -> None:
        ...
        self._value_map_for_vis.fill(0)
    ...
    def update_map(
        ...
        curr_map = self._localize_new_data(depth, tf_camera_to_episodic, min_depth, max_depth, fov) # 将当前帧的可见区域置信度权重矩阵置于全局dense map下

        # Fuse the new data with the existing data
        self._fuse_new_data(curr_map, values) # 完成两个地图融合
    ...
    def sort_waypoints(
        self, robot_xy, waypoints: np.ndarray, radius: float, reduce_fn: Optional[Callable] = None
        ...
        def get_value(point: np.ndarray) -> Union[float, Tuple[float, ...]]:
            x, y = point
            px = int(x * self.pixels_per_meter) + self._episode_pixel_origin[1] # row
            py = int(-y * self.pixels_per_meter) + (1000-self._episode_pixel_origin[0]) # col
            point_px = (px, py)
            all_values = [
        ...
        # # new
        # def get_value_weight(point, robot_xy) -> Union[float, Tuple[float, ...]]:
        #     x, y = point
        ...
        #     value_score = pixel_value_within_radius(self._value_map[..., 0], point_px, radius_px)
        #     robot_frontier_dis = ((robot_xy[0]-point[0])**2+(robot_xy[1]-point[1])**2)**0.5
        #     final_score = value_score/(1+0.5*robot_frontier_dis)

        #     return final_score


        values = [get_value(point) for point in waypoints]
        # values = [get_value_weight(point, robot_xy) for point in waypoints]
    ...
    def visualize(
        ...
        return map_img, self._camera_positions, self._last_camera_yaw
    ...
    def _process_local_data(self, depth: np.ndarray, fov: float, min_depth: float, max_depth: float) -> np.ndarray:
        ...
        # Get blank cone mask
        # # 生成一个带有置信度权重的视场角(FOV)锥形掩码，中心区域的置信度更高，边缘区域的置信度更低（矩阵保存）
        cone_mask = self._get_confidence_mask(fov, max_depth) # 只计算一次
    ...
    def _localize_new_data(
        ...
        max_depth: float,
        fov: float,
    ) -> np.ndarray: # 将当前帧的可见区域置信度权重矩阵置于全局dense map下
        # Get new portion of the map
        curr_data = self._process_local_data(depth, fov, min_depth, max_depth) # 可见区域的置信度权重
        ...
        px = int(cam_x * self.pixels_per_meter) + self._episode_pixel_origin[1] # res_row
        py = int(-cam_y * self.pixels_per_meter) + (1000-self._episode_pixel_origin[0]) # res_col
    ...
    def _get_blank_cone_mask(self, fov: float, max_depth: float) -> np.ndarray: # 在一个圆上截取 [90° - fov/2, 90° + fov/2] 的扇形区域
    ...
    def _get_confidence_mask(self, fov: float, max_depth: float) -> np.ndarray: # 生成一个带有置信度权重的视场角(FOV)锥形掩码，中心区域的置信度更高，边缘区域的置信度更低（矩阵保存）
        ...
        cone_mask = self._get_blank_cone_mask(fov, max_depth) # 在一个圆上截取 [90° - fov/2, 90° + fov/2] 的扇形区域
    ...
    def _fuse_new_data(self, new_map: np.ndarray, values: np.ndarray) -> None:
        ...
        if self._obstacle_map is not None:
            ...
            new_map[explored_area == 0] = 0 # 只保留已探索的空闲区域的当前有效权重
            self._map[explored_area == 0] = 0 # 只保留已探索的空闲区域的累计有效权重
            self._value_map[explored_area == 0] *= 0 # 只保留已探索的空闲区域的累计有效value
        ...
        if self._use_max_confidence: # 默认为False
            ...
            self._value_map = self._value_map * weight_1_channeled + values * weight_2_channeled # 累计的语义map
            self._map = self._map * weight_1 + new_map * weight_2 # 累计的置信度map
