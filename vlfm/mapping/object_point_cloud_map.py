...
class ObjectPointCloudMap:
    ...
    def update_map(
        ...
        print("before_local_cloud:", len(local_cloud))
        
        if len(local_cloud) == 0:
            return

        print("after_local_cloud:", len(local_cloud))
        ...
        # newest_debug: 过滤掉不在真值附近的点云
        local_cloud_center = np.mean(local_cloud, axis=0)
        world_cx, world_cy, world_cz, world_turn = get_current_world_pos(habitat_env)
        if(turn_id==0):
            world_turn += 0
        elif(turn_id==1): # 左转
            world_turn += 0.5*np.pi
        elif(turn_id==2): # 背后
            world_turn += np.pi
        elif(turn_id==3): # 右转
            world_turn -= 0.5*np.pi
        res_loc_in_real_world = get_absolute_pos_world(local_cloud_center[0], -local_cloud_center[1], world_cx, world_cy, world_turn)
        min_real_goal_dis = 10000000000
        for temp_goal in habitat_env.current_episode.goals:
            temp_dis = ((temp_goal.position[2]-res_loc_in_real_world[0])**2+(temp_goal.position[0]-res_loc_in_real_world[1])**2)**0.5
            if temp_dis<min_real_goal_dis:
                min_real_goal_dis = temp_dis
        if(min_real_goal_dis)>2:
            return 
        if ((local_cloud_center[2]+0.88+world_cz) >= (world_cz+2.0)) and ((object_name=="chair") or (object_name=="bed") or (object_name=="toilet") or (object_name=="couch")):
            return 
        if ((local_cloud_center[2]+0.88+world_cz) < world_cz-1.0):
            return
        ...
        print("final_clouds:", self.clouds)
