...
class ObjectPointCloudMap:
    ...
    def get_semantic_map(self, reliable_cluster_ls, object_name, size: int = 1000):
        self.semantic_map = np.zeros((size, size), np.uint8)

        if(len(reliable_cluster_ls)>0):
            best_matched_cluster = self.get_best_matched_cluster(reliable_cluster_ls, object_name)
