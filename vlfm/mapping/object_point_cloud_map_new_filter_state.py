...
class ObjectPCCluster:
    def __init__(self):
        ...
        self.has_in_semantic_map = False
...
class ObjectPointCloudMap:
    ...
    def __init__(self, erosion_size: float, size: int = 1000) -> None:
        ...
        # =====> new_add <=====
        self.semantic_map = np.zeros((size, size), np.float32)
    ...
    # # zhou_percept （新考虑）
    # def get_semantic_map(self, cluster_ls, object_name, size: int = 1000): # 传入的cluster_ls需要包括历史已经传入过的
    #     self.semantic_map = np.zeros((size, size), np.float32)
    #     for temp_cluster in cluster_ls:
    #         navigate_target_cloud = self.get_cluster_target_cloud(temp_cluster, object_name)
    #         if(len(navigate_target_cloud)>0):
    #             xy_points = navigate_target_cloud[:, :2]
    #             pixel_points = self._xy_to_px(xy_points) # 点云的在map栅格上的像素坐标
    #             self.semantic_map[pixel_points[:, 1], pixel_points[:, 0]] = temp_cluster.object_dict_per_cluster[object_name].confidence_score # 计算语义地图（1为相应类别）

    # filter_state
    def get_semantic_map(self, cluster_ls, object_name, size: int = 1000):
        self.semantic_map = np.zeros((size, size), dtype=np.float32)
        best_matched_cluster = self.get_best_matched_cluster_new_sort(cluster_ls, object_name)
        if (best_matched_cluster is not None):
            best_matched_cluster.has_in_semantic_map = True
