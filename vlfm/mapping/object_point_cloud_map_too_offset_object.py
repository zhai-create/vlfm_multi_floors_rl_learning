    ...
    def update_map(
        ...
        if dist < 1.0: # 过滤掉太近的object
            ...
            if(len(global_cloud)>0):
                new_mean_point = np.mean(global_cloud[:, :2], axis=0)
                new_object_frontier = find_closest_vector(new_mean_point, global_cloud[:, :2])
            else:
                new_object_frontier = None
            return new_object_frontier
        else: # new_object_revise_frontier
            if(len(global_cloud[global_cloud[..., -1] != 1][:, :2])>0):
                new_mean_point = np.mean(global_cloud[global_cloud[..., -1] != 1][:, :2], axis=0)
                new_object_frontier = find_closest_vector(new_mean_point, global_cloud[global_cloud[..., -1] != 1][:, :2])
            else:
                new_object_frontier = None
        ...
        return new_object_frontier # new_object_revise_frontier
