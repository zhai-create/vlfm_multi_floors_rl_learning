...
class BaseMap:
    ...
    def __init__(self, size: int = 1000, pixels_per_meter: int = 20, *args: Any, **kwargs: Any):
        ...
        self._episode_pixel_origin = np.array([size // 2, size // 2]) # 先列后行
    ...
    def reset(self) -> None:
        ...
        self._traj_vis = TrajectoryVisualizer(np.array([1000-self._episode_pixel_origin[1], 1000-self._episode_pixel_origin[0]]), self.pixels_per_meter)
        # self._traj_vis = TrajectoryVisualizer(self._episode_pixel_origin, self.pixels_per_meter)
    ...
    def _xy_to_px(self, points: np.ndarray) -> np.ndarray:
        ...
        """
        # res_col = 1000-origin_col-y*20
        # res_row = x*20+origin_row
