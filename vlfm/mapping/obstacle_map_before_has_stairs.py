...
class ObstacleMap(BaseMap):
    ...
    def update_map(
        ...
        min_depth: float,
        max_depth: float,
        fx: float, # image_width / (2 * np.tan(camera_fov_rad / 2))
        fy: float,
        topdown_fov: float,
        explore: bool = True,
        ...
        if update_obstacles:
            ...
            mask = scaled_depth < max_depth # 过滤掉最大值的mask（过滤掉真实的最大值+过滤掉空洞）
            point_cloud_camera_frame = get_point_cloud(scaled_depth, mask, fx, fy) # 相机坐标系下的过滤后的点云坐标
            point_cloud_episodic_frame = transform_points(tf_camera_to_episodic, point_cloud_camera_frame) # episodic_frame坐标系下的点云
            obstacle_cloud = filter_points_by_height(point_cloud_episodic_frame, self._min_height, self._max_height) # 按照高度过滤点云(0.15--0.88)
            ...
            self._map[pixel_points[:, 1], pixel_points[:, 0]] = 1 # 计算障碍区域地图（1为被占用）
